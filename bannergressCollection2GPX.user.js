// ==UserScript==
// @name         bannergressCollection2GPX
...
downloadBtn.innerText = 'Download GPX Collection';
...
downloadBtn.style.right = '0'; // 紧贴页面左侧
...
function generateGPX (missionData) {
  ...
  for (let i = 0; i < Object.keys(missionData).length; i++) {
    const title = escapeXml(missionData[i].title);
    const trackIndex = creator.addTrack(`Mission ${i + 1} ${title}`);
    const stepsData = missionData[i].steps;
    stepsData.forEach(element => {
      const poi = element.poi;
      if (poi && poi.latitude !== undefined && poi.longitude !== undefined) {
        creator.addTrackpoint(trackIndex, poi.latitude, poi.longitude, generateRandomDecimal(400, 5));
      }
    });
  }
